layui.use(['table', 'layer'], function() { /*function(){}是回调函数*/
    ...
    table.render({
        elem: '#musictable',
        height: 600,
        url: 'http://localhost:8080/tbUser/queryAllByLimit' //数据接口
        ,
        title: '用户列表',
        page: true //开启分页
        ,
    ...
    table.on('tool(musicfilter)', function(obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'del') {
            ...
            layer.confirm('真的删除行么', function(index) {
                ...
                $.ajax({
                    type: "DELETE", //请求方式
                    url: "http://localhost:8080/tbUser/deleteById/" + data.id,
    ...
    table.on('edit(musicfilter)', function(obj) { //update tb_music set name=1 where id=data.id
        ...
        if(data.id!=1) {
            ...
            $.ajax({
                type: "PATCH",
                url: "http://localhost:8080/tbUser/updateuser",
